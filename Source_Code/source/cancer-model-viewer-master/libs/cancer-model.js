 ...
 */
 ...
 (function () {
    ...
    var replicate_ncells_values = [];            // an array an array of the cumulative number of cell count events for each replicate - index is tick number
    ...
    var ncells_mean = [];                        // the mean cell number values at each tick
    ...
    var ncells_standard_deviation = [];          // standard deviation at each tick of the cell count values
    ...
    var gene_font_size = 24;                        // default font size

    var input_node_font_size  = 32;

    var output_node_font_size = 32;
    ...
    var initialise = function () {
        ...
        var display_mean_label = function (event_type) {
            return "Mean cumulative " + event_type + " averaged over " + replicates.length + " simulation runs";
        };
        var display_all_label = function (event_type) {
            return "cumulative " + event_type + " for all " + replicates.length + " simulation runs";
        };
        ...
        replicates.forEach(function (replicate, index) {
            ...
            var initial_cell_count = replicate.l[1].length / 2;
            ...
            for (tick = 1; tick < replicate.l.length; tick++) {
                ...
                ncells_values[tick] = initial_cell_count + current_proliferation - current_apoptosis; 
            }
            if (last_tick < replicate.l.length) {
                last_tick = replicate.l.length;
            }
            replicate_states.push(cell_states);
            replicate_ncells_values.push(ncells_values);
        ...
        for (tick = 1; tick < last_tick; tick++) {
            ...
            ncells_mean[tick] = mean(tick, replicate_ncells_values);
            ...
            ncells_standard_deviation[tick] = standard_deviation(tick,replicate_ncells_values, ncells_mean);
        }
     
        if (ncells_mean[ncells_mean.length-1]) {
           display_mean("mean-ncells", display_mean_label("number of cells"), ncells_mean, ncells_standard_deviation);
           display_all( "all-ncells",  display_all_label ("number of cells"), replicate_ncells_values, ncells_mean, ncells_standard_deviation);
        }
     
        
       if (proliferation_mean[proliferation_mean.length-1]) {
            // not all zeros
            display_mean("mean-proliferation", display_mean_label("proliferation events"), proliferation_mean, proliferation_standard_deviation);
            display_all( "all-proliferation",  display_all_label ("proliferation events"), replicate_proliferation_values);
        }
        
        if (apoptosis_mean[apoptosis_mean.length-1]) {
            display_mean("mean-apoptosis", display_mean_label("apoptosis events"), apoptosis_mean, apoptosis_standard_deviation);
            display_all( "all-apoptosis",  display_all_label ("apoptosis events"), replicate_apoptosis_values, apoptosis_mean, apoptosis_standard_deviation);
        }
        if (growth_arrest_mean[growth_arrest_mean.length-1]) {
            display_mean("mean-growth_arrest", display_mean_label("growth arrest events"), growth_arrest_mean, growth_arrest_standard_deviation);
            display_all( "all-growth_arrest",  display_all_label ("growth arrest events"), replicate_growth_arrest_values, growth_arrest_mean, growth_arrest_standard_deviation);
        }
        if (necrosis_mean[necrosis_mean.length-1]) {
            display_mean("mean-necrosis", display_mean_label("necrosis events"), necrosis_mean, necrosis_standard_deviation);
            display_all( "all-necrosis",  display_all_label ("necrosis events"), replicate_necrosis_values, necrosis_mean, necrosis_standard_deviation);
        }
