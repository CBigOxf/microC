<xsl:stylesheet version="2.0"
	...
	<xsl:template match="gxl">
		...
		<graphml>
			...
			<key id="MY-PARAMETER" for="node" attr.name="MY-PARAMETER" attr.type="string" />
...
<xsl:otherwise>all</xsl:otherwise>
	...
	<xsl:template match="node">
		...
		<node id="{$id}">
			...
			<xsl:for-each select="../node[@id=$id]/parameter">
				<xsl:variable name="parameter" select="@idActiveInteractions" />
     			<xsl:variable name="parameter1" select="replace($parameter, string-join( (':', $id), ''), '')"/>
				<xsl:variable name="parameter2" select="substring($parameter1, 2)"/>
				<xsl:variable name="parameter3" select="string-join( ('(', $parameter2,')'), '')"/>
				<xsl:variable name="parameter4" select="replace($parameter3, ' ', ' and ')"/>				
				<data key="MY-PARAMETER"><xsl:value-of select="$parameter4"/></data>
			</xsl:for-each>
