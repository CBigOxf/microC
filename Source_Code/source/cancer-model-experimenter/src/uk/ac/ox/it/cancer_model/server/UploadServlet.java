...
public class UploadServlet extends HttpServlet {
		...
		try {
			...
			for (FileItem item : items) {
				if (item.isFormField()) {
					...
					// add unique tokens around each file name since this will be wrapped up in
					// minimal HTML
					writer.print(
							"file-name-token" + fileName + "file-name-token" + tempFile.toString() + "file-name-token");
				}
			}
	...
	void transformToGraphML(String inputFileName, String outputFileName) throws SaxonApiException {
		// based upon
		// https://www.cs.duke.edu/courses/fall08/cps116/docs/saxon/samples/java/S9APIExamples.java
		Processor proc = new Processor(false);
		XsltCompiler comp = proc.newXsltCompiler();
//        XsltExecutable exp = comp.compile(new StreamSource(new File("GXL-to-GraphML-new.xsl")));
//        XsltExecutable exp = comp.compile(new StreamSource(new File("/Program Files/Apache Software Foundation/Tomcat 7.0/webapps/ROOT/GXL-to-GraphML-new.xsl")));  //fix for Tomcat instance on local machine
		XsltExecutable exp = comp
				.compile(new StreamSource(new File("/usr/share/tomcat/webapps/microc/GXL-to-GraphML-new.xsl"))); // this
																													// is
																													// for
																													// merlin
																													// server
